...
// Bug: bare d='M'... in strict module -> ReferenceError: d is not defined
...
// Replace any existing boardLinks definition (fixed or buggy) with HELPER
function replaceBoardLinks(h) {
  const start = h.indexOf('function boardLinks');
  if (start < 0) return null;
  const end = h.indexOf('function nz', start);
  if (end < 0) throw new Error('boardLinks found but no function nz after it');
  return h.slice(0, start) + HELPER + h.slice(end);
}
...
function hasBareD(h) {
  ...
  // Bare statement: ;d='M'  (fixed form uses ,d='M' inside const list)
  return fn.includes(";d='M'") || fn.includes(';d="M"');
}
...
function patch(file) {
  ...
  const hasOverlay = h.indexOf('className:"board-links"') >= 0;
  const hasFn = h.indexOf('function boardLinks') >= 0;
  const bare = hasFn && hasBareD(h);
  const exact = hasFn && h.indexOf(HELPER) >= 0;

  if (hasFn && exact && hasOverlay) {
    console.log('  boardLinks already fixed');
  } else if (hasFn) {
    const next = replaceBoardLinks(h);
    if (next == null) throw new Error(file + ': replace failed');
    h = next;
    console.log('  boardLinks replaced' + (bare ? ' (bare d fixed)' : ''));
    if (!hasOverlay) {
      if (h.split(OLD_IMG).length - 1 !== 1) throw new Error(file + ': img anchor count');
      h = h.replace(OLD_IMG, NEW_IMG);
      console.log('  overlay div inserted');
    }
  } else {
  ...
  if (h.indexOf(HELPER) < 0) throw new Error(file + ': fixed HELPER missing');
  ...
  if (hasBareD(h)) throw new Error(file + ': bare d still present');
  if (h.indexOf(";d='M'") >= 0 || h.indexOf(';d="M"') >= 0) throw new Error(file + ': bare d= still present');
...
// Strict-mode unit test (module context) — must catch bare d
const html = fs.readFileSync('index.html', 'utf8');
const i = html.indexOf('function boardLinks');
const j = html.indexOf('function nz', i);
const fnSrc = html.slice(i, j);
const hg = { 92: 88, 75: 67, 57: 43, 39: 22, 23: 17 };
const bg = { 21: 41, 31: 50, 35: 47, 62: 82, 66: 75, 69: 72, 86: 95, 90: 91, 79: 82 };
const svg = eval('(function(){"use strict";var hg=' + JSON.stringify(hg) + ';var bg=' + JSON.stringify(bg) + ';' + fnSrc + ';return boardLinks()})()');
if (!svg || svg.indexOf('<svg') < 0) throw new Error('strict eval failed');
if (svg.indexOf('path') < 0) throw new Error('no paths in svg');
console.log('strict eval OK', svg.length);
console.log('ALL OK');
